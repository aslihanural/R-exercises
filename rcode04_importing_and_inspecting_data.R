...
# confusing / technical at times); the goal is to improve one's understanding
# of a function
...
# note: don't have to be fully write out argument names (as long as this is
...
# frequency table by default does not show missing values
...
subset(dat, smoke == "yes", smokenum, drop=TRUE)
mean(subset(dat, smoke == "yes", smokenum, drop=TRUE), na.rm=TRUE)
...
mean(subset(dat, smoke == "yes" & age >= 30, smokenum, drop=TRUE), na.rm=TRUE)
