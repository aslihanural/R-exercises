...
# most dataset are just a bunch of vectors (of the same length) combined into
...
# can click on 'dat' to view the contents (useful for larger datasets)
#
...
# note: putting a character variable (e.g., sex, grp) into a data frame turns
# it into a 'factor' (more on factors later)

# subsetting (first value is the row, second is the column)
...
# for data frame, we can also use a special notation to select columns

dat[2]
dat[c(1,4)]

# note: now taking a single column returns a data frame with that column
...
# if you want to make a permanent selection, you have to assign this output
# to an object (either overwrite the original object or make new ones)
...
# any more further in the script (but don't do this now!)
#
# dat <- subset(dat, sex == "Male")

# subset() can select rows and one or multiple columns

subset(dat, sex == "Male", age)
subset(dat, sex == "Male", age, drop=TRUE)
subset(dat, sex == "Male", c(age, sex))
...
# variable, but all of the other variables are unchanged!
...
# can avoid this with the 'na.rm' argument (set it to TRUE); then the
# mean/sum is taken over the non-missing values within each row

dat$ymean <- rowMeans(dat[c("y1","y2")], na.rm=TRUE)
dat

# subsetting when there are missings
