...
#
# this means the size of the plotting device is too small to accommodate the
# plot you are trying to create; this can happen more easily in RStudio
# because the pane for plots at the bottom right is a bit small; if this
# happen, you just have to make the size of the plotting pane larger
...
# how did I know the hex values for 'skyblue'?
...
plot(density(dat$age, adjust=2), main="Distribution of Age")
...
# plot x and y values from 'res' with point sizes proportional to the number
# of points that overlap at a particular location

plot(res$x, res$y, cex=res$number, pch=19, xlab="Stress", ylab="PA")

# make the point sizes a bit smaller

plot(res$x, res$y, cex=res$number*0.4, pch=19, xlab="Stress", ylab="PA")

# note: adjusting 'cex' is akin to adjusting the radius, but the area of a
# circle is proportional to the radius squared; so to make the point sizes
# proportional to the number of overlapping points, need to take the square
# root of the number of overlapping points

plot(res$x, res$y, cex=sqrt(res$number)*0.6, pch=19, xlab="Stress", ylab="PA")
...
# note: the jittering is random, so each time you rerun the command above,
# the plot will look (slightly) different; to make the look of the plot fully
...
# a small digression on installing packages; the usual command for this is
# install.packages("package name"); but if you put this in your script, then
# running the entire script (re)installs the package, which can be annoying;
# we can do the following to only install the package if it isn't already
# installed

# install if necessary the 'hexbin' package
...
plot(dat$pss, dat$posaff, xlab="Stress", ylab="PA", pch=16, col=rgb(0,0,0,.2))

# note: using pch=16, since this point symbol doesn't have a border
...
# save a plot (in various formats) (saved to current working directory)
...
# load the 'MASS' package (comes with R)
...
dat2 <- dat[c("pss","posaff")]
dat2 <- na.omit(dat2)

res <- kde2d(dat2$pss, dat2$posaff, n=100)
res
...
persp(res, xlab="Stress", ylab="Positive Affect", zlab="Density",
      ...
      theta=25, phi=25, shade=0.7, ltheta=90)
...
persp(res, xlab="Stress", ylab="Positive Affect", zlab="Density",
      col=color[facetcol], theta=65, phi=35, border="gray20")
...
rm(dat2, res, nrz, ncz, nbcol, color, zfacet, facetcol)

############################################################################

# scatterplot with smoothed densities
...
par(mar=c(9,4,5,2))
barplot(table(dat$marital), las=2, main="(b) Martial Status")
...
# after we have created a plot with a function like plot(), we can add
# further elements (points, lines, text, etc.) to the plot as needed
